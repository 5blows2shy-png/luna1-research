...
const svg = (body, viewBox = "0 0 512 512") => `<svg xmlns="http://www.w3.org/2000/svg" viewBox="${viewBox}" role="img" aria-labelledby="title desc"><title id="title">Klyro</title><desc id="desc">A dark triangular prism with a subtle geometric monogram and spectral color lines.</desc>${body}</svg>`;
const wordmark = (fill = palette.ink, x = 590, y = 292, size = 104) => `<text x="${x}" y="${y}" fill="${fill}" font-family="Arial, Helvetica, sans-serif" font-size="${size}" font-weight="700" letter-spacing="18">KLYRO</text><text x="${x + 5}" y="${y + 62}" fill="${fill}" opacity=".62" font-family="Arial, Helvetica, sans-serif" font-size="26" font-weight="600" letter-spacing="12">RESEARCH</text>`;
...
const poster = svg(`<rect width="1600" height="900" fill="${palette.ink}"/><circle cx="800" cy="430" r="360" fill="#151818"/><text x="95" y="94" fill="${palette.paper}" font-family="Arial, Helvetica, sans-serif" font-size="25" font-weight="700" letter-spacing="8">KLYRO</text><text x="95" y="720" fill="${palette.paper}" font-family="Arial, Helvetica, sans-serif" font-size="68" font-weight="500">Price is the result.</text><text x="95" y="800" fill="${palette.paper}" opacity=".64" font-family="Arial, Helvetica, sans-serif" font-size="42">Klyro reveals the causes.</text><g transform="translate(544 100) scale(1.48)">${prism()}</g>`, "0 0 1600 900");
for (const [name, contents] of new Map([["klyro-prism.svg", icon], ["klyro-prism-black.svg", iconBlack], ["klyro-prism-white.svg", iconWhite], ["klyro-logo-horizontal.svg", horizontal], ["klyro-logo-stacked.svg", stacked]])) await fs.writeFile(path.join(brandDir, name), contents);
...
await Promise.all([render(icon, path.join(brandDir, "klyro-prism-transparent.png"), 1024, 1024), render(horizontal, path.join(brandDir, "klyro-logo-horizontal.png"), 1280, 440), render(stacked, path.join(brandDir, "klyro-logo-stacked.png"), 1000, 720), render(social, path.join(brandDir, "klyro-social-card.png"), 1200, 630), render(poster, path.join(brandDir, "klyro-commercial-poster.png"), 1600, 900), render(icon, path.join(appDir, "icon.png"), 512, 512), render(icon, path.join(appDir, "apple-icon.png"), 180, 180)]);
