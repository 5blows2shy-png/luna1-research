  ...
  return <>
    ...
    <section>
      ...
      <div id="portfolio-tab-panel" className="portfolio-tab-panel" role="tabpanel" aria-labelledby={`tab-${activeTab.toLowerCase().replaceAll(" ", "-")}`}>
        ...
        {activeTab === "Watchlist" && <div className="table-wrap watchlist-table-wrap"><table className="watchlist-table"><caption>Research watchlist · manually maintained educational research priorities</caption><thead><tr><th>Ticker</th><th>Company name</th><th>LUNA Score</th><th>Research status</th><th>Watchlist note</th><th>Setup status</th><th>Primary catalyst</th><th>Primary risk</th></tr></thead><tbody>{watchlist.map((item) => <tr key={item.ticker}><td data-label="Ticker"><b>{item.ticker}</b></td><td data-label="Company name">{item.company}</td><td data-label="LUNA Score"><b className="watchlist-score" aria-label={`LUNA Score ${item.score} out of 100`}>{item.score}/100</b></td><td data-label="Research status"><span className="status">{item.researchStatus}</span></td><td data-label="Watchlist note" className="portfolio-copy">{item.note}</td><td data-label="Setup status">{item.setupStatus}</td><td data-label="Primary catalyst" className="portfolio-copy">{item.catalyst}</td><td data-label="Primary risk" className="portfolio-copy">{item.risk}</td></tr>)}</tbody></table></div>}
