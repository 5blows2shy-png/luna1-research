...
export async function GET(request: Request) {
  ...
  const result = await getFmpResource("sec-filings", { symbol: search.get("symbol") ?? undefined, page: search.get("page") ?? 0, limit: Math.min(Number(search.get("limit") ?? 20) || 20, 50) }, { ttlMs: 5 * 60_000 });
