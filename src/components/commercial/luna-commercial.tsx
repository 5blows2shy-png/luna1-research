...
import { Expand, FastForward, Pause, Play, RotateCcw, Volume2, VolumeX } from "lucide-react";
...
function sceneForTime(time: number, duration: number) { let scene = 0; const scale = duration / commercialCuts.feature.duration; featureStarts.forEach((start, index) => { if (time >= start * scale) scene = index; }); return scene; }

export function LunaCommercial({ compact = false, media, cut = compact ? "trailer" : "feature" }: { compact?: boolean; media?: CommercialMedia; cut?: CommercialCut }) {
  const reducedMotion = useReducedMotion();
  const duration = commercialCuts[cut].duration;
  ...
  const activeScene = reducedMotion ? 5 : sceneForTime(time, duration);

  const pause = useCallback(() => { setPlaying(false); video.current?.pause(); }, []);
  const play = useCallback(() => { if (reducedMotion) return; if (time >= duration) setTime(0); setPlaying(true); if (video.current) video.current.play().catch(() => setPlaying(false)); }, [reducedMotion, time, duration]);
  ...
  useEffect(() => { if (!playing || hasVideo) return; const timer = window.setInterval(() => { setTime(current => { const next = Math.min(current + .1, duration); if (next >= duration) window.setTimeout(() => setPlaying(false), 0); return next; }); }, 100); return () => window.clearInterval(timer); }, [playing, hasVideo, duration]);

  const scenes = [<MarketNoiseIntro active reduced={Boolean(reducedMotion)} key="market"/>, <PrismLightAnimation active key="prism"/>, <ProductScreenMontage active key="products"/>, <CommandCenterFrame active key="command"/>, <FounderIdentityFrame active key="founder"/>, <ClosingLogoFrame active key="closing"/>];
  ...
  return <div ref={player} className={`commercial-player${compact ? " commercial-player--compact" : ""}`}>
    ...
    <div className="commercial-controls" aria-label="Commercial controls">
      ...
      {!compact && <button onClick={() => seek(duration * .845)} aria-label="Skip intro"><FastForward/><span>Skip intro</span></button>}
