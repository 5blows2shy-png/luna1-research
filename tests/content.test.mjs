...
test("founder card and portrait appear on the resume",()=>{const about=fs.readFileSync("src/app/about/page.tsx","utf8");const resume=fs.readFileSync("src/app/resume/page.tsx","utf8");assert.doesNotMatch(about,/shyheim-lee-founder.jpeg/);assert.match(resume,/src="\/shyheim-lee-founder.jpeg"/);assert.match(resume,/alt="Portrait of Shy Lee, founder of Luna1 Research"/);assert.match(resume,/Infrastructure, Compounders, Inflections, Bottle Neck Constraint Analysis/);assert.ok(fs.existsSync("public/shyheim-lee-founder.jpeg"))});

test("market commentary is hidden without deleting its archived routes",()=>{const data=fs.readFileSync("src/lib/data.ts","utf8");const home=fs.readFileSync("src/app/page.tsx","utf8");assert.ok(!data.includes('label: "Market Commentary"'));assert.ok(!home.includes("Market journal"));assert.ok(fs.existsSync("src/app/market-commentary/page.tsx"));assert.ok(fs.existsSync("src/app/market-commentary/[slug]/page.tsx"))});
