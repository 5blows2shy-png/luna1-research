...
test("portfolio reflects approved public positions",()=>{const source=fs.readFileSync("src/app/portfolios/page.tsx","utf8");for(const ticker of ["WWD","AMAT","GS","PDFS"])assert.ok(!source.includes(`ticker:\"${ticker}\"`),`${ticker} remains active`);for(const [ticker,price] of [["CASY","824.00"],["PANW","272.54"],["WELL","237.21"]]){assert.ok(source.includes(`ticker:\"${ticker}\"`),`${ticker} is missing`);assert.ok(source.includes(`entryPrice:${price}`),`${ticker} entry price is missing`)}assert.ok(!source.includes("<th>Shares</th>"));assert.ok(!source.includes("<th>Cost basis</th>"));assert.ok(!source.includes("Closed Positions"));assert.match(source,/ticker:\"AAPL\"/)});
